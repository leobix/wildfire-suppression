...
function full_formulation(integer_routes, region_data, constraint_data, rotation_order,
    ...
    @constraint(m, fire_flow[c=1:C, g=1:G, t=1:T, rest=1:2], sum(z[constraint_data.f_out[c, g, t, rest]]) ==
                                                             sum(z[constraint_data.f_in[c, g, t, rest]]))

    @constraint(m, base_flow[c=1:C, t=1:T, rest=1:2], sum(z[constraint_data.b_out[c, t, rest]]) ==
                                                      sum(z[constraint_data.b_in[c, t, rest]]))


    @constraint(m, linking[g=1:G, t=1:T], LINE_PER_CREW * sum(z[constraint_data.supp_fire[g, t]]) >= l[g, t]
    )


    # build start constraint
    @constraint(m, start[c=1:C], sum(z[constraint_data.start[c]]) == 1
    )
...
function price_branch(col_gen_config, route_data, supp_plan_data, region_data, rotation_order, gamma, price_branch)

    @assert price_branch == true
    ...
    if price_branch
        ...
        @variable(m, A[c=1:NUM_CREWS, g=1:NUM_FIRES, t=1:NUM_TIME_PERIODS], Int)
    ...
    # constraint for A
    @constraint(m, crew_at_fire_time[c=1:NUM_CREWS, g=1:NUM_FIRES, t=1:NUM_TIME_PERIODS],
        A[c, g, t] <= sum(route[c, r] * route_data.fires_fought[c, r, g, t] for
                          r = 1:route_data.routes_per_crew[c])
    )
...
function init_route_subproblem(crew_ixs, crew, constraint_data, integer_routes=false)
    ...
    @constraint(m, fire_flow[g=1:G, t=1:T, rest=1:2], sum(z[constraint_data.f_out[crew, g, t, rest]]) ==
                                                      sum(z[constraint_data.f_in[crew, g, t, rest]]))

    @constraint(m, base_flow[t=1:T, rest=1:2], sum(z[constraint_data.b_out[crew, t, rest]]) ==
                                               sum(z[constraint_data.b_in[crew, t, rest]]))

    # build start constraint
    @constraint(m, start, sum(z[constraint_data.start[crew]]) == 1
    )
...
function full_network_flow(region_data, constraint_data, rotation_order,
    ...
    @constraint(m, fire_linking[fire=1:NUM_FIRES, t=1:NUM_TIME_PERIODS], d[fire, t] >= sum(crews_needed[i] * y[i] for i in fire_linking_arcs[fire][t]))
    ...
    @constraint(m, route_linking[g=1:G, t=1:T], sum(z[constraint_data.supp_fire[g, t]]) >= d[g, t]
    )
