...
function single_DCG_node(test_features, data)
    ...
    current_num_routes = Base.copy(col_gen_data.routes.routes_per_crew)
    current_num_plans = Base.copy(col_gen_data.suppression_plans.plans_per_fire)
    ...
    while (~opt) & (n_iters < max_iters) & (ts["cg"] < max_time)
        ...
        current_num_routes = Base.copy(next_num_routes)
        current_num_plans = Base.copy(next_num_plans)
