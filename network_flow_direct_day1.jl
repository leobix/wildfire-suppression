...
function full_network_flow(
	...
	if has_values(m)  # Only read solution values when the solver finished cleanly.
		...
		# @info "Crew arc selections" begin
		# 	crew_logs = Dict{Int,Vector{Dict{Symbol,Any}}}()
		# 	for crew ∈ 1:num_crews
		# 		vals = vec(value.(crew_vars[crew]))
		# 		sel = findall(>(1e-6), vals)
		# 		entries = Vector{Dict{Symbol,Any}}()
		# 		for ix ∈ sel
		# 			push!(entries, Dict(
		# 				:index => ix,
		# 				:value => vals[ix],
		# 				:cost => crew_models[crew].arc_costs[ix],
		# 				:data => crew_models[crew].long_arcs[ix, :],
		# 			))
		# 		end
		# 		crew_logs[crew] = entries
		# 	end
		# 	crew_logs
		# 	println(crew_logs)
		# end
...
dataset = joinpath(@__DIR__, "..", "ai_wildfire", "fire_models_2018_08_01_prev_1117")
...
crew_models, _ = build_crew_models_from_empirical(
      ...
      num_time_periods,
      crew_speed;
      crew_gaccs = target_gaccs,              # crews still come from the original GACC list
      fire_gaccs = filtered_gaccs,            # but restrict which fires appear
      fires_by_gacc = day_one_subset,
      fire_folder = dataset,
  )
...
# Jacob's old experiments

# num_crews = 10  # Baseline crew count for a single exploratory solve.
# num_fires = 3  # Baseline number of simultaneous fires.
# num_time_periods = 14  # Planning horizon length in periods.
# crew_speed = 640.0  # Travel speed parameter fed into crew network generation.
...
# 	num_time_periods,
# 	crew_speed,
# )  # Generate crew networks for the baseline scenario.
...
# 	num_time_periods,
# 	20,
# )  # Generate fire networks for the baseline scenario (20 line-per-crew parameter).
...
# full_network_flow(crew_models, fire_models, verbose = false)  # Perform one warm-up solve on the baseline instance.


# # sizes = [(3, 10, 14, 20), (6, 20, 14, 20), (9, 30, 14, 20), (12, 40, 14, 20), (15, 50, 14, 20), (12, 40, 14, 20), (15, 50, 14, 20), (18, 60, 14, 20), (21, 70, 14, 20)]  # Scenario tuples (fires, crews, horizon, line per crew).
# sizes = [(3, 10, 14, 20)]  # Scenario tuples (fires, crews, horizon, line per crew).

# for (num_fires, num_crews, num_time_periods, line_per_crew) ∈ sizes  # Loop over the benchmark scenarios.

# 	#Ryne added
# 	@info "Scenario" num_fires=num_fires num_crews=num_crews num_time_periods=num_time_periods line_per_crew=line_per_crew crew_speed=crew_speed
...
# 		num_time_periods,
# 		crew_speed,
# 	)  # Generate crew networks for the current scenario.
...
# 		num_time_periods,
# 		line_per_crew,
# 	)  # Generate fire networks for the current scenario.
...
# 		time_limit = 60.0,
# 	)
# 	linear_outputs[num_crews] = Dict("ub" => u, "lb" => l, "time" => t)  # Store LP relaxation performance for this crew count.

# 	#Ryne added
# 	@info "LP results" crew_count=num_crews ub=u lb=l time=t
...
# 		time_limit = 1200.0,
# 	)
# 	integer_outputs[num_crews] = Dict("ub" => u, "lb" => l, "time" => t)  # Store integer performance for the same crew count.

# 	#Ryne added
# 	@info "MIP results" crew_count=num_crews ub=u lb=l time=t
# end

# open(args["directory_output"] * "output.json", "w") do f  # Write the collected metrics to disk.
# 	JSON.print(f, Dict("linear" => linear_outputs, "integer" => integer_outputs), 4)  # Emit nicely formatted JSON output.
# end
