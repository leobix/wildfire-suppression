...
# This script mirrors the single-shot network_flow_direct.jl solver but wraps it
# inside the same rolling-horizon loop used by EmpiricalMain.jl. Every day we
# freeze arcs that have already been executed, re-solve the full network-flow
# MIP for the remaining horizon, export per-day artifacts, and then advance the
# horizon one day forward.
...
function full_network_flow(
    ...
    fire_vars = Vector{Vector{VariableRef}}(undef, num_fires)
    ...
    @constraint(
        m,
        fire_flow[fire = 1:num_fires, t = 1:num_times, s = 1:size(fire_models[fire].state_out_arcs, 1)],
    ...
    if has_values(m)
        ...
        fire_selected = Vector{Vector{Int64}}(undef, num_fires)
        fire_selected_values = Vector{Vector{Float64}}(undef, num_fires)
        for fire in 1:num_fires
...
function rolling_horizon_network_flow()
    ...
    for t in 0:num_time_periods
        ...
        # horizon exactly.
        fixed_fire = build_fixed_arc_values(fire_arcs_by_time, committed_fire_arcs, t)
